...
#            (성적 1등, 2등, 3등) 높낮이는 알 수 있지만 어느정도 차인지 알수없음.
# 양적척도
# - 등간척도: 관찰대상의 속성을 상대적 크기로 나타냄
#            순위를 부여 할 뿐만 아니라 어느 정도 큰지 숫자간의 의미가 있음
#            (온도) 가산이 가능함. 하지만 절대적이지 않음.
# - 비율척도: 절대적 기준이 있는 영점이 존재하고 모든 사칙연산이 가능
#            연구대상 차이를 비교할 수 있으며 순위를 만들 수 있음(판매량, 점수)

# 널 값 체크
print('널값 체크', df.isnull().sum())
# 성별 나이평균
print('성별 나이 평균')
# 생존,사망 인원수
...
# df['Pclass_label'].values_counts().plot(kind='pie', autopct='%.2f%%')
# plt.show()
